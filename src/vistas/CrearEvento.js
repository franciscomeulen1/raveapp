...
function CrearEvento() {
  ...
  const handleCrearEvento = async () => {
    try {
      ...
      // 2. Crear artistas nuevos si hay
      ...
      // 3. Crear o usar fiesta recurrente
      ...
      const responseEvento = await api.post('/Evento/CrearEvento', nuevoEvento);
      const idEventoCreado = responseEvento.data.idEvento;

      console.log('Evento creado correctamente. ID:', idEventoCreado);

      // 6. Obtener el evento con las fechas y sus idFecha
      ...
      // 7. Enviar entradas por cada fecha
  ...
  //     // Obtener inicio y fin de venta de entradas según el array de fechas
  //     const inicioVenta = fechas[0]?.fechaIncioVenta || '';
  //     const finVenta = fechas[fechas.length - 1]?.fechaFinVentaGeneral || '';
