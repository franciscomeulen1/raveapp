...
export default function BotonDeArrepentimiento({
  ...
  // 👇 banderas de validación "viejas"
  const [isAfter10Days, setIsAfter10Days] = useState(false);
  const [isLessThan48h, setIsLessThan48h] = useState(false);

  // 👇 NUEVO: ventana de 5 días desde la última modificación del evento
  const [canRefundByUpdateWindow, setCanRefundByUpdateWindow] = useState(false);
  const [hasUpdateWindowReason, setHasUpdateWindowReason] = useState(false);

  // cuando abro, limpio estados y recalculo condiciones
  useEffect(() => {
    if (open) {
      setChecked(false);
      setLoading(false);
      ...
      setCanRefundByUpdateWindow(false);
      setHasUpdateWindowReason(false);
      ...
      let compraDate = null;
      if (fechaCompraISO) {
        const tmpCompra = new Date(fechaCompraISO);
        if (!Number.isNaN(tmpCompra.getTime())) {
          compraDate = tmpCompra;
          const diffMs = now.getTime() - tmpCompra.getTime();
      ...
      // 3) 👇 NUEVO: ventana de 5 días desde dtUpdate
      //    Si hubo una modificación después de la compra y dentro de los últimos 5 días,
      //    el usuario puede pedir reembolso aunque no cumpla las reglas de 10 días / 48h.
      if (evento?.dtUpdate) {
        const upd = new Date(evento.dtUpdate);
        if (!Number.isNaN(upd.getTime())) {
          let updateAplicaParaEstaCompra = true;

          // Si tenemos fecha de compra, solo dejamos aplicar la ventana
          // si la actualización es igual o posterior a la compra.
          if (compraDate) {
            if (upd.getTime() < compraDate.getTime()) {
              updateAplicaParaEstaCompra = false;
            }
          }

          if (updateAplicaParaEstaCompra) {
            const diffMsFromUpdate = now.getTime() - upd.getTime();
            const diffDaysFromUpdate =
              diffMsFromUpdate / (1000 * 60 * 60 * 24);

            // dentro de los 5 días corridos desde la modificación
            if (diffMsFromUpdate >= 0 && diffDaysFromUpdate <= 5) {
              setCanRefundByUpdateWindow(true);
              setHasUpdateWindowReason(true);
            }
          }
        }
      }
  ...
  // 👇 canRefund ahora contempla la ventana por actualización
  const canRefund = canRefundByUpdateWindow || (!isAfter10Days && !isLessThan48h);
      ...
      <div className="bg-base-100 rounded-2xl w-full max-w-xl p-6 shadow-xl space-y-5">
        ...
        {successData ? (
          ...
          <>
            ...
            {/* 👇 Si NO se puede reembolsar por ninguna vía, mostramos el motivo y NO mostramos el checkbox */}
            {!canRefund ? (
              <>
              ...
              <>
                {/* 👇 Mensaje aclaratorio si el reembolso es posible gracias a la actualización */}
                {hasUpdateWindowReason && (isAfter10Days || isLessThan48h) && (
                  <div className="rounded-lg bg-info/10 text-info text-sm p-3 mb-2">
                    Podés solicitar el reembolso porque hubo una modificación reciente en los datos del evento
                    dentro de los últimos <strong>5 días</strong>.
                  </div>
                )}
