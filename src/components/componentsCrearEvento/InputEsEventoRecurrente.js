...
const InputEsEventoRecurrente = ({ onSeleccionRecurrente }) => {
    ...
    useEffect(() => {
        if (esRecurrente) {
            const fiestaExistente = fiestas.find(f => f.idFiesta === fiestaSeleccionada);
            const nueva = nuevaFiesta.trim();

            const validacion = !!(fiestaExistente || nueva);

            onSeleccionRecurrente({
                esRecurrente: true,
                idFiesta: fiestaExistente?.idFiesta || null,
                nombreFiestaNueva: fiestaExistente ? null : nueva || null,
                valido: validacion,
            });
        } else {
