...
const InputUbicacionEvento = ({ onUbicacionChange, ubicacionInicial }) => {
  ...
  // helper para capitalizar cada palabra sin eliminar espacios
  ...
  useEffect(() => {
    if (ubicacionInicial?.provincia?.nombre) {
      const cargarDesdeInicial = async () => {
        ...
        // si viene con dirección previa, la capitalizamos una vez
        setDireccion(ubicacionInicial.direccion ? toTitleCase(ubicacionInicial.direccion) : '');
  ...
  // 2.b RESELECCIONAR municipio/localidad cuando ya tengo las listas
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <div className="form-control w-full max-w-xs mt-4">
        <label className="label">
          <span className="label-text font-semibold text-lg flex flex-wrap items-center gap-1">
            Dirección <span className="text-red-500">*</span>
            <span className="text-gray-400 text-xs">(Debes ingresar el nombre de la calle y el número)</span>
          </span>
        </label>
        <input
          type="text"
          ...
          onChange={(e) => {
            ...
            const capitalizado = toTitleCase(valor);
            setDireccion(capitalizado);
          }}
        />
        {!direccion && (
          <span className="text-red-500 text-sm mt-1">
