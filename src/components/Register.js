...
const Register = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'telefono') {
      ...
      const limitados = soloNumeros.slice(0, 10);       // máximo 10 caracteres
      setFormData((f) => ({ ...f, telefono: limitados }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.telefono.length !== 10) {
      setError('El teléfono debe tener exactamente 10 dígitos (sin 0 ni 15).');
      return;
    }
            ...
            <form className="space-y-3 mt-4" onSubmit={handleSubmit}>
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="input input-bordered w-full"
                  placeholder="1155115511"
                  required
                  inputMode="numeric"      // teclado numérico en mobile
                  maxLength={10}           // como extra, aunque ya lo limitamos en handleChange
                />
                <p className="text-sm text-gray-500 mt-1">
                  Debes ingresar tu celular sin 0 ni 15, por ejemplo: 11-5511-5511
                </p>
              </label>
